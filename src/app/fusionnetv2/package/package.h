...
namespace GryFlux
{
    ...
    {
    public:
        FusionImagePackage(const cv::Mat &visible, const cv::Mat &infrared, int idx, std::string filename)
            : visible_(visible.clone()),
              infrared_(infrared.clone()),
              idx_(idx),
    ...
    {
    public:
        FusionRunnerPackage(const cv::Mat &fusedY, int idx, std::string filename)
            : fusedY_(fusedY.clone()), idx_(idx), filename_(std::move(filename)) {}
    ...
    {
    public:
        FusionResultPackage(const cv::Mat &image, int idx, std::string filename)
            : image_(image.clone()), idx_(idx), filename_(std::move(filename)) {}
