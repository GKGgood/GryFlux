...
namespace GryFlux
{
    namespace
    {
        ...
        inline float determineInputScaling(const rknn_tensor_attr &attr)
        {
            ...
            const bool affineLike = (attr.qnt_type == RKNN_TENSOR_QNT_AFFINE_ASYMMETRIC || attr.qnt_type == RKNN_TENSOR_QNT_NONE);
            if (!affineLike)
            {
                return 1.0f;
            }

            if (attr.qnt_type == RKNN_TENSOR_QNT_AFFINE_ASYMMETRIC && std::fabs(attr.scale - 1.0f) < 1e-4f && attr.zp == 0)
            {
                return 255.0f;
            }
...
#define RKNN_CHECK(op, msg)                                                                                              \
    ...
    void RkRunner::prepareTensorAttributes()
    {
        ...
        for (std::size_t i = 0; i < inputAttrs_.size(); ++i)
        {
            ...
            const float scaling = determineInputScaling(attr);
            inputScaling_.push_back(scaling);
            if (scaling != 1.0f)
            {
                LOG.info("[RkRunner] Applying %.1fx scaling to input %zu to match FP16 expectations", scaling, i);
            }
    ...
    void RkRunner::copyInputData(const cv::Mat &mat, std::size_t index)
    {
        ...
        case RKNN_TENSOR_FLOAT16:
        {
            ...
            std::memcpy(tensorMem->virt_addr, halfMat.ptr<std::uint16_t>(), elemCount * sizeof(std::uint16_t));
            break;
        }
        case RKNN_TENSOR_INT8:
        {
    ...
    std::shared_ptr<DataObject> RkRunner::process(const std::vector<std::shared_ptr<DataObject>> &inputs)
    {
        ...
        return std::make_shared<FusionRunnerPackage>(fusedY,
                                 prepPackage->get_id(),
                                 prepPackage->get_filename());
    }
}
