    ...
    : device_id_(device_id)
{
    ...
    if (!model_stream)
    {
        throw std::runtime_error("Failed to open RKNN ReID model: " + model_path);
    }
    ...
    if (length <= 0)
    {
        throw std::runtime_error("Empty RKNN ReID model: " + model_path);
    }
    ...
    if (!model_stream)
    {
        throw std::runtime_error("Failed to read RKNN ReID model: " + model_path);
    }
    ...
    if (io_num.n_input != 1)
    {
        throw std::runtime_error("ReidContext only supports single-input RKNN models");
    }
    if (io_num.n_output < 1)
    {
        throw std::runtime_error("ReID RKNN model has no outputs");
    }
    ...
    {
        throw std::runtime_error("ReidContext only supports NHWC RKNN input tensors");
    }
    ...
    {
        throw std::runtime_error("ReID RKNN input shape mismatch");
    }

    input_type_ = input_attr_.type;
    if (input_type_ != RKNN_TENSOR_INT8 && input_type_ != RKNN_TENSOR_FLOAT16)
    {
        throw std::runtime_error("ReID RKNN input tensor type must be int8 or fp16");
    }
    ...
    if (!input_mem_)
    {
        throw std::runtime_error("rknn_create_mem(reid_input) failed");
    }
    checkRknnCall(rknn_set_io_mem(ctx_, input_mem_, &io_input_attr_), "rknn_set_io_mem(reid_input)");
    ...
    for (std::size_t i = 0; i < output_attrs_.size(); ++i)
    {
        ...
        {
            throw std::runtime_error("ReID RKNN output tensor type must be int8/fp16/fp32");
        }

        output_mems_[i] = rknn_create_mem(ctx_, tensorByteSize(io_attr));
        if (!output_mems_[i])
        {
            throw std::runtime_error("rknn_create_mem(reid_output) failed");
        }
        checkRknnCall(rknn_set_io_mem(ctx_, output_mems_[i], &io_attr), "rknn_set_io_mem(reid_output)");
    }
}
...
void ReidContext::setInput(const uint8_t *data, std::size_t size)
{
    ...
    {
        throw std::runtime_error("Invalid ReID RKNN input tensor byte size");
    }
    ...
    checkRknnCall(rknn_mem_sync(ctx_, input_mem_, RKNN_MEMORY_SYNC_TO_DEVICE), "rknn_mem_sync(reid_input)");
}

void ReidContext::runInference()
{
    checkRknnCall(rknn_run(ctx_, nullptr), "rknn_run(reid)");
    for (std::size_t i = 0; i < output_mems_.size(); ++i)
    {
        checkRknnCall(
            rknn_mem_sync(ctx_, output_mems_[i], RKNN_MEMORY_SYNC_FROM_DEVICE),
            "rknn_mem_sync(reid_output)");
    }
}

std::pair<void *, std::size_t> ReidContext::getOutput(std::size_t output_index)
{
    if (output_index >= output_mems_.size())
    {
        throw std::out_of_range("ReID output index out of range");
    }
    return {output_mems_[output_index]->virt_addr, output_attrs_[output_index].n_elems};
}

const rknn_tensor_attr &ReidContext::getOutputAttr(std::size_t index) const
{
...
float ReidContext::deqntAffineToF32(int8_t qnt, int zp, float scale) const
{
    return dequantizeValue(qnt, zp, scale);
}

rknn_core_mask ReidContext::toCoreMask(int device_id)
{
    if (device_id < 0)
    {
        device_id = 0;
...
std::size_t ReidContext::tensorTypeSize(rknn_tensor_type type)
{
    switch (type)
    {
    ...
    default:
        throw std::runtime_error("Unsupported ReID RKNN tensor type");
    }
}

std::size_t ReidContext::tensorByteSize(const rknn_tensor_attr &attr)
{
    return static_cast<std::size_t>(attr.n_elems) * tensorTypeSize(attr.type);
}

void ReidContext::releaseResources()
{
    if (input_mem_)
    {
    ...
    std::size_t instance_count)
{
    ...
    auto probe_context = std::make_shared<ReidContext>(model_path, device_id);
    ...
    {
        contexts.push_back(std::make_shared<ReidContext>(
            model_path,
            static_cast<int>(i),
            model_width,
            model_height));
