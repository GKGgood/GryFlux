...
YoloxImageDirSource::YoloxImageDirSource(const std::string &datasetDir)
    : datasetDir_(datasetDir)
{
    ...
    LOG.info("YoloxImageDirSource initialized: dir=%s files=%zu",
             datasetDir_.string().c_str(),
             imageFiles_.size());
}

std::unique_ptr<GryFlux::DataPacket> YoloxImageDirSource::produce()
{
    while (cursor_ < imageFiles_.size())
    {
        ...
        auto packet = std::make_unique<YoloxPacket>();
        packet->idx = idx_++;
...
bool YoloxImageDirSource::isSupportedImage(const fs::path &path)
{
